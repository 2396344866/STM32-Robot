...
extern int Sensors_I2C_WriteRegister(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data);
extern int Sensors_I2C_ReadRegister(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data);
extern void Delay_ms(uint32_t ms);
extern void get_tick_count(unsigned long *count);

#define i2c_write   Sensors_I2C_WriteRegister
#define i2c_read    Sensors_I2C_ReadRegister 
#define delay_ms    Delay_ms
#define get_ms      get_tick_count
#define log_i       BSP_MPL_LOGI
#define log_e       BSP_MPL_LOGE
#ifndef min
    ...
    for (ii = 0; ii < length; ii += this_write) {
        this_write = min(LOAD_CHUNK, length - ii);
				if (mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii])) {
						printf("Error: Failed to write memory at offset %d.\n", ii);
						printf("Write address: %d, Write length: %d\n", ii, this_write);
						printf("Write data: ");
						for (int j = 0; j < this_write; j++) {
								printf("%02X ", firmware[ii + j]);
						}
						printf("\n");
						return -1;
				}
				if (mpu_read_mem(ii, this_write, cur)) {
						printf("Read address: %d, Read length: %d\n", ii, this_write);
						return -1;
				}
