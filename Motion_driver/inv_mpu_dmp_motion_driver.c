...
//######################自己添加@@@@@@@@@@@@@@@@@@@@@@@@@@@
/**
 * @brief 将标准 3x3 方向矩阵转换为 DMP 硬件可识别的标量配置字。
 *
 * 该函数通过降维映射，将 9 字节的旋转矩阵压缩为 16 位的标量（Scalar）。
 * DMP 内部使用该标量来校准传感器坐标系与物理安装坐标系之间的变换关系。
 *
 * 编码逻辑基于行向量特征提取：
 * - 矩阵被视为 3 个行向量（Row Vectors），分别对应转换后的 X、Y、Z 轴分量。
 * - 每个行向量通过 inv_row_2_scale() 映射为一个 3-bit 的特征值（包含轴索引和符号位）。
 * - 最终标量通过位移操作将 3 个特征值打包。
 *
 * @param[in] mtx  指向 3x3 旋转矩阵的指针（线性存储的 9 元素 signed char 数组）。
 * 通常由单位矩阵经过排列组合（+/-1, 0）构成，表示正交变换。
 * - mtx[0..2]: 第一行 (Row 1) -> 对应输出的低 3 位
 * - mtx[3..5]: 第二行 (Row 2) -> 对应输出的中 3 位
 * - mtx[6..8]: 第三行 (Row 3) -> 对应输出的高 3 位
 *
 * @return unsigned short 打包后的方向掩码（Orientation Mask/Scalar）。
 * Bit [2:0] : Row 1 特征值 (X轴映射)
 * Bit [5:3] : Row 2 特征值 (Y轴映射)
 * Bit [8:6] : Row 3 特征值 (Z轴映射)
 */
unsigned short inv_orientation_matrix_to_scalar(const signed char *mtx)
{
    ...
    /* * 处理矩阵第一行 (Row 1)
     * 计算该行向量对应的缩放/符号特征，占据标量的低 3 位 (Bit 0-2)。
     */
    scalar = inv_row_2_scale(mtx);

    /* * 处理矩阵第二行 (Row 2)
     * 指针偏移 +3 跳至第二行起始位置。
     * 计算特征值后左移 3 位，写入标量的 Bit 3-5。
     */
    scalar |= inv_row_2_scale(mtx + 3) << 3;

    /* * 处理矩阵第三行 (Row 3)
     * 指针偏移 +6 跳至第三行起始位置。
     * 计算特征值后左移 6 位，写入标量的 Bit 6-8。
     */
